...
public:
    ...
    inline void setNdotV(real ndotv)
    {
        _ndotv = ndotv;
    }
...
public:
    ...
    float _tb;     // The silhouette ending point's coordinates are : _woeb[0]+tb*(_woeb[1]-_woeb[0])
    bool _front;
...
public:
    ...
    inline void setSmoothEdge(WXSmoothEdge * sEdge)
    {
        _pSmoothEdge = sEdge;
    }
...
public:
    ...
    inline bool front(bool useConsistency=false)
    {
        if(useConsistency && frs_materialIndex() < 4)
            return (frs_materialIndex() == 0 || frs_materialIndex() == 2);
        return _front;
    }

    inline bool consistent()
    {
        return front(true) == front(false);
    }
